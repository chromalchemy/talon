...
class Actions:
    ...
    def rebelle_nrep_eval(clj: str):
        ...
        cmd = (
            f"{CLJ_NREPL_EVAL_PATH} -p 7888 <<'EOF'\n"
            "(in-ns 'main)\n"
            f"{clj}\n"
            "EOF\n"
        )
    ...
    def invoker_clj_eval(params: str):
        """use invoker in rebelle project to call fn"""
        nvk_path = "/Users/ryan/.local/bin/nvk"
        cmd = f"cd '{CODE_PATH}' && {nvk_path} {params} "
        actions.user.system_command_nb(cmd)
